<template>
  <div class="min-h-screen">
    ...
    <div class="py-8 px-4 lg:px-6 max-w-7xl mx-auto">
      ...
      <div v-else-if="error" class="text-center py-20 text-red-400">Xatolik: {{ error.message }}</div>
      ...
      <!-- Grid View -->
      <div v-else-if="view === 'grid'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <NuxtLink v-for="b in filteredBuildings" :key="b.id" :to="`/buildings/${b.slug}`" class="card card-hover overflow-hidden">
          ...
          <div class="p-5">
            ...
            <div class="grid grid-cols-3 gap-2 text-xs">
              <div>
                <div class="text-ink-600">Qavatlar</div>
                <div class="font-medium">{{ b.floorsCount }}</div>
              </div>
              <div>
                <div class="text-ink-600">Unitlar</div>
                <div class="font-medium">{{ b.totalUnits }}</div>
              </div>
              <div>
                <div class="text-ink-600">Maydon</div>
                <div class="font-medium">{{ formatArea(b.totalArea) }} m²</div>
              </div>
            </div>
          </div>
        </NuxtLink>
      ...
      <div v-if="filteredBuildings.length > 0" class="mt-6 text-sm text-ink-500 text-center">
        {{ filteredBuildings.length }} bino topildi
      </div>
    </div>
  </div>
</template>
