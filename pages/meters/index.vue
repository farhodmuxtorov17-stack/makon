  ...
  <div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3">
      <div>
        ...
        <p class="text-sm text-ink-500 mt-1">{{ filtered.length }} hisoblagich</p>
      </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
      <div class="card p-4 flex items-center gap-3">
        ...
        <div>
          <p class="text-xl font-bold">{{ countByType('ELECTRICITY') }}</p>
          <p class="text-xs text-ink-500">Elektr</p>
        </div>
      </div>
      <div class="card p-4 flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-info-50 flex items-center justify-center">
          <svg class="w-5 h-5 text-info-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              ...
              stroke-linejoin="round"
              stroke-width="2"
              d="M20.49 15a9 9 0 11-2.12-9.36L23 10M15 14l-3 3-3-3"
            />
          </svg>
        </div>
        <div>
          <p class="text-xl font-bold">{{ countByType('WATER') }}</p>
          <p class="text-xs text-ink-500">Suv</p>
        </div>
      </div>
      <div class="card p-4 flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-danger-50 flex items-center justify-center">
          ...
          >
            <path
              ...
              stroke-linejoin="round"
              stroke-width="2"
              d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.24 17 5.343 19.24 5.58 21 7.5 21 10c0 .997-.5 2-1 3"
            />
          </svg>
        </div>
        <div>
          <p class="text-xl font-bold">{{ countByType('GAS') }}</p>
          <p class="text-xs text-ink-500">Gaz</p>
        </div>
      </div>
      <div class="card p-4 flex items-center gap-3">
        ...
        <div>
          <p class="text-xl font-bold">{{ countByType('HEAT') }}</p>
          <p class="text-xs text-ink-500">Issiqlik</p>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row gap-3">
      <div class="flex-1">
        <SearchInput v-model="search" placeholder="Kod yoki seriya bo'yicha..." />
      </div>
    ...
    <div class="card hidden md:block">
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="m in pagedMeters" :key="m.id">
              ...
              <td>
                <span class="badge badge-info">{{ typeLabel(m.type) }}</span>
              </td>
              <td class="font-mono text-xs text-ink-500">{{ m.serialNumber }}</td>
              <td class="text-ink-500">{{ buildingName(m.buildingId) }}</td>
              <td class="text-ink-500">{{ m.unitId || '—' }}</td>
              <td>
                <span :class="m.isActive ? 'badge badge-success' : 'badge badge-neutral'">{{
                  m.isActive ? 'Faol' : 'Nofaol'
                }}</span>
              </td>
              <td class="text-ink-500 text-xs">{{ formatDate(m.installedAt) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <EmptyState
        ...
        title="Hisoblagichlar topilmadi"
    ...
    <div class="md:hidden space-y-3">
      <div v-for="m in pagedMeters" :key="m.id" class="card p-4">
        ...
        <div class="flex items-center justify-between mt-1">
          <span class="badge badge-info">{{ typeLabel(m.type) }}</span>
          <span class="font-mono text-xs text-ink-400">{{ m.serialNumber }}</span>
        </div>
        <p class="text-xs text-ink-500 mt-2">
          {{ buildingName(m.buildingId) }} · {{ m.unitId || '—' }}
        </p>
      </div>
      <EmptyState
        v-if="!filtered.length"
    ...
    <div class="card">
      <div class="px-5 py-4 border-b border-ink-100">
        <h3 class="font-semibold">So'nggi o'qishlar</h3>
      </div>
      <div class="table-wrapper">
        <table class="table">
          <thead>
          ...
          <tbody>
            <tr v-for="r in readings.slice(0, 10)" :key="r.id">
              ...
              <td class="text-ink-500">{{ formatDate(r.readingAt) }}</td>
  ...
  if (search.value)
    r = r.filter(
      (m) =>
        ...
        m.serialNumber?.toLowerCase().includes(search.value.toLowerCase())
    )
  return r
})
...
function countByType(t: string) {
  return meters.value.filter((m) => m.type === t).length
}
