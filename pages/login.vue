<template>
  <div class="min-h-screen flex bg-white">
    ...
    <div class="flex-1 flex flex-col lg:max-w-[520px] xl:max-w-[560px] relative z-10 bg-white">
      ...
      <div class="px-8 lg:px-12 py-7">
        <NuxtLink to="/" class="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div class="text-[9px] text-ink-400 font-medium tracking-widest uppercase mt-0.5">
      ...
      <div class="flex-1 flex items-center justify-center px-8 lg:px-12 pb-8">
        <div class="w-full max-w-[380px]">
          <h1 class="font-display text-[28px] font-bold text-ink-900 mb-2 tracking-tight">
            Tizimga kirish
          </h1>
          <p class="text-ink-500 text-sm mb-8">
            Boshqaruv paneliga kirish uchun identifikatsiyadan oʻting
          </p>
          ...
          <div v-if="activeTab === 'eri'" class="space-y-4">
            <div class="flex items-start gap-3 p-3.5 rounded-xl bg-ink-50 border border-ink-200/60">
              <div
                class="w-8 h-8 rounded-lg bg-ink-900 flex items-center justify-center flex-shrink-0"
              >
                <ShieldCheck :size="16" :stroke-width="2" class="text-white" />
              </div>
              <div class="pt-0.5">
                <div class="text-ink-900 text-sm font-semibold leading-tight">
                  Elektron raqamli imzo
                </div>
                <div class="text-ink-500 text-xs mt-0.5">
                  ERI orqali xavfsiz kirish — E-xujjat.uz
                </div>
              </div>
            </div>
            <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="label">STIR</label><input v-model="form.stir" class="input" placeholder="305 123 456" />
              </div>
              <div>
                <label class="label">Telefon</label><input v-model="form.phone" class="input" placeholder="+998 90 123 45 67" />
              </div>
            </div>
            <div>
              <label class="label">Foydalanuvchi</label><input v-model="form.user" class="input" placeholder="Azizbek Karimov" />
            </div>
            ...
            <div class="flex items-center justify-between">
              <label class="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  type="checkbox"
                  class="w-4 h-4 rounded border-ink-300 text-brand-600 focus:ring-brand-200"
                />
          ...
          <!-- Telegram -->
          <button
            class="w-full flex items-center justify-center gap-2.5 py-3 rounded-xl border border-ink-200 hover:border-ink-300 hover:bg-ink-50 transition-all duration-200"
          >
          ...
          <div class="mt-6 p-3 rounded-xl bg-ink-50 border border-ink-200/40">
            <p class="text-xs text-ink-500 text-center leading-relaxed">
              <span class="font-semibold text-ink-700">Demo:</span>
              <code class="text-brand-600">admin@makon.uz</code> (boshqaruv) ·
              <code class="text-brand-600">tenant@makon.uz</code> (ijarachi)
            </p>
          </div>

          <div class="mt-6 text-center text-xs text-ink-400">© 2026 MAKON Real Estate</div>
        </div>
      </div>
    </div>
    ...
    <div class="hidden lg:block flex-1 relative overflow-hidden">
      <img
        :src="asset('/tashkent/city-night.jpg')"
        alt="Tashkent City"
      ...
      <div class="absolute inset-0 flex flex-col justify-between p-12">
        ...
        <div class="flex items-start justify-between">
          <div
            class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20"
          >
        ...
        <div class="max-w-md">
          ...
          <p class="text-white/60 text-sm leading-relaxed max-w-sm">
            Binolar, shartnomalar, toʻlovlar va servislar — bitta tizimda. Toshkentning yetakchi
            biznes-markazlari uchun moʻljallangan.
          </p>
          <div class="flex items-center gap-6 mt-6">
            <div
              v-for="p in platformBadges"
              :key="p.label"
              class="flex items-center gap-2 text-white/50 text-xs font-medium"
            >
