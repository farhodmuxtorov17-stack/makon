      ...
      <div class="h-full flex flex-col admin-sidebar">
        ...
        <nav class="flex-1 overflow-y-auto py-2 px-2.5 tg-hide-scrollbar">
          <template v-for="(group, gi) in groupedNav" :key="gi">
            ...
            <NuxtLink
              ...
              :to="item.to"
              class="flex items-center gap-3 px-3 py-2.5 rounded-[10px] text-[13px] transition-all relative group"
        ...
        <div class="p-2.5 flex-shrink-0">
          <button @click="handleLogout" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-[12px] text-sm admin-sidebar-item transition-all duration-200 group">
            ...
            <LogOut :size="15" class="text-ink-400 transition-transform group-hover:translate-x-1" />
          </button>
        </div>
      </div>
    </aside>
    ...
    <div class="flex-1 min-w-0 flex flex-col relative z-10">
      <header class="h-[60px] sticky top-0 z-20 glass border-b border-black/5 dark:border-white/5 flex items-center px-4 lg:px-6 gap-3 flex-shrink-0">
        ...
        <div class="hidden md:flex items-center gap-2 text-sm">
          <span class="text-ink-400 font-medium">{{ currentRoleShort }}</span>
