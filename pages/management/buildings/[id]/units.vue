    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div class="kpi-strip kpi-strip--blue">
        ...
        <div class="kpi-strip__body">
          ...
          <div class="kpi-strip__label">Umumiy maydon</div>
        </div>
      </div>
    </div>
...
import { ArrowLeft, Plus, Search, Link, Tag, History, X , Building2, CheckCircle, FileText, Grid3x3} from 'lucide-vue-next'
...
const occupiedCount = computed(() => units.value.filter((u: any) => u.status === "OCCUPIED" || u.status === "RENTED").length)
const vacantCount = computed(() => units.value.filter((u: any) => u.status === "VACANT").length)
const totalArea = computed(() => units.value.reduce((s: number, u: any) => s + (u.area || 0), 0).toLocaleString("ru-RU"))
