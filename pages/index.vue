  ...
  <div class="min-h-screen bg-white text-slate-900" style="font-family: 'Satoshi', 'Inter', system-ui, sans-serif;">
    ...
    <header class="sticky top-0 z-50 bg-white border-b border-slate-100">
      <div class="max-w-[1200px] mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <button class="hidden sm:block text-sm font-medium text-slate-600 hover:text-slate-900 px-3 py-2 rounded-lg hover:bg-slate-50 transition-colors">Mening e'lonlarim</button>
          <button class="text-sm font-medium text-slate-600 hover:text-slate-900 px-3 py-2 rounded-lg border border-slate-200 hover:border-slate-300 transition-colors">E'lon joylash</button>
    ...
    <section class="py-10 px-4">
      <div class="max-w-[840px] mx-auto text-center">
        ...
        <p class="text-slate-400 text-sm mb-8">Minglab e'lonlar, yangi binolar va ishonchli egalari — bitta platformada</p>

        <!-- Search bar -->
        <div class="bg-white rounded-2xl border border-slate-200 shadow-lg shadow-slate-100 p-2 text-left">
          <!-- Deal type tabs -->
    ...
    <section class="max-w-[1200px] mx-auto px-4 py-8">
      <div class="flex items-center justify-between mb-4">
        ...
        <NuxtLink to="/" class="text-sm text-blue-600 hover:text-blue-700 font-medium transition-colors">Hammasini ko'rish →</NuxtLink>
      </div>
      <div class="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4" style="scrollbar-width: thin;">
        <div v-for="b in newBuildings" :key="b.name" class="flex-shrink-0 w-[280px] bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-xl hover:shadow-slate-100 transition-all cursor-pointer group">
          ...
          <div class="p-4">
            ...
            <NuxtLink :to="`/catalog/${b.id}`" class="block text-center text-sm font-semibold text-blue-600 hover:text-blue-700 py-2.5 rounded-xl border border-blue-200 hover:border-blue-300 hover:bg-blue-50 transition-all">
              Batafsil →
            </NuxtLink>
          </div>
        </div>
    ...
    <section class="max-w-[1200px] mx-auto px-4 py-8">
      ...
      <div class="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4" style="scrollbar-width: thin;">
        <div v-for="p in buyListings" :key="p.id" class="flex-shrink-0 w-[260px] bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-xl hover:shadow-slate-100 transition-all cursor-pointer group">
          ...
          <div class="px-3.5 pb-3.5">
            <NuxtLink :to="`/catalog/${p.id}`" class="block text-center text-sm font-semibold text-slate-700 hover:text-blue-600 py-2.5 rounded-xl border border-slate-200 hover:border-blue-300 transition-all">
              Batafsil →
            </NuxtLink>
          </div>
        </div>
    ...
    <section class="max-w-[1200px] mx-auto px-4 py-8">
      ...
      <div class="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4" style="scrollbar-width: thin;">
        <div v-for="p in rentListings" :key="p.id" class="flex-shrink-0 w-[260px] bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-xl hover:shadow-slate-100 transition-all cursor-pointer group">
          <NuxtLink :to="`/catalog/${p.id}`">
            <div class="relative h-36 overflow-hidden">
              <img :src="asset(p.img)" :alt="p.title" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              <div class="absolute top-2 right-2 text-xs font-medium text-white bg-black/50 backdrop-blur-sm px-2 py-0.5 rounded-full">{{ p.photos }} foto</div>
          ...
          <div class="px-3.5 pb-3.5 flex gap-2">
            <NuxtLink :to="`/catalog/${p.id}`" class="flex-1 text-center text-sm font-semibold text-slate-700 hover:text-blue-600 py-2.5 rounded-xl border border-slate-200 hover:border-blue-300 transition-all">
              Batafsil →
            </NuxtLink>
            <button @click="openApplication(p)" class="flex-1 text-sm font-semibold text-white py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 transition-all shadow-sm">
              Ijaraga olish
            </button>
          </div>
        </div>
    ...
    <section class="max-w-[1200px] mx-auto px-4 py-8">
      <div class="relative rounded-3xl overflow-hidden bg-gradient-to-br from-indigo-600 to-blue-700 p-10 md:p-14">
        ...
        <div class="relative z-10 max-w-lg">
          <h3 class="text-white text-2xl md:text-3xl font-bold mb-3" style="font-family: 'Clash Display', sans-serif;">Mulkingizni boshqarish — oson va qulay</h3>
          <p class="text-blue-100 text-sm mb-6 leading-relaxed">MAKON platformasi orqali binolar, shartnomalar, to'lovlar va xizmatlarni avtomatlashtiring. 16+ modul, real vaqt rejimida boshqaruv.</p>
    ...
    <div v-if="showAppModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" @click="showAppModal = false"></div>
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div v-if="appSubmitted" class="p-8 text-center">
          ...
          <NuxtLink to="/login" class="block text-center text-sm font-semibold text-white py-3.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 transition-all">
            Tizimga kirish →
          </NuxtLink>
          <button @click="showAppModal = false; appSubmitted = false" class="mt-3 text-sm text-slate-400 hover:text-slate-600">Yopish</button>
        </div>
        ...
        <div v-else>
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <div>
              ...
              <p v-if="selectedProperty" class="text-xs text-slate-400 mt-0.5">{{ selectedProperty.title }}</p>
            </div>
            <button @click="showAppModal = false" class="text-slate-400 hover:text-slate-600 text-2xl leading-none">×</button>
          </div>
          <div class="p-6 space-y-4">
            <div>
              <label class="text-sm font-medium text-slate-700 mb-1.5 block">F.I.Sh. <span class="text-rose-500">*</span></label>
              <input v-model="appForm.name" type="text" placeholder="Ism Familiya Sharifingiz"
            ...
            <div>
              ...
              <input v-model="appForm.phone" type="tel" placeholder="+998 90 123 45 67"
                class="w-full text-sm bg-slate-50 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white transition-all" />
            </div>
            <div>
            ...
            <div>
              ...
              <input v-model="appForm.company" type="text" placeholder="MCHJ / LLC / Yakka tartibdagi tadbirkor"
                class="w-full text-sm bg-slate-50 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white transition-all" />
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-sm font-medium text-slate-700 mb-1.5 block">Ijara muddati</label>
                <select v-model="appForm.duration" class="w-full appearance-none text-sm bg-slate-50 rounded-xl px-4 py-3 outline-none cursor-pointer focus:ring-2 focus:ring-blue-500/20">
                  <option>3 oy</option><option>6 oy</option><option>1 yil</option><option>2 yildan ortiq</option>
                </select>
              </div>
              <div>
                <label class="text-sm font-medium text-slate-700 mb-1.5 block">Boshlash sanasi</label>
                <input v-model="appForm.startDate" type="date"
                  class="w-full text-sm bg-slate-50 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white transition-all" />
              </div>
            </div>
            <div>
              <label class="text-sm font-medium text-slate-700 mb-1.5 block">Qo'shimcha izoh</label>
              <textarea v-model="appForm.notes" rows="3" placeholder="Ijara shartlari, qo'shimcha talablar..."
                class="w-full text-sm bg-slate-50 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white transition-all resize-none"></textarea>
            </div>
            <button @click="submitApplication" :disabled="!appForm.name || !appForm.phone"
            ...
            <p class="text-xs text-slate-400 text-center">Yuborish orqali siz foydalanuvchi shartnomasiga rozilik bildirasiz</p>
          </div>
        </div>
      </div>
    </div>
...
const appForm = reactive({
  name: '', phone: '', email: '', company: '', duration: '1 yil', startDate: '', notes: ''
})
...
const newBuildings = [
  { id: 1, name: 'Nest One', location: 'Yashnabad, Tashkent City', commute: 'Metro 5 daqiqa', price: '850 mln so\'m', developer: 'BI Group', img: '/tashkent/nest-one.jpg' },
  { id: 2, name: 'Finance Center', location: 'Mirobad, Amir Temur', commute: 'Metro 8 daqiqa', price: '420 mln so\'m', developer: 'Murad Buildings', img: '/tashkent/finance-center.jpg' },
  { id: 3, name: 'Mall Towers', location: 'Chilonzor, Bunyodkor', commute: 'Metro 3 daqiqa', price: '380 mln so\'m', developer: 'Akay', img: '/tashkent/mall-towers.jpg' },
  { id: 4, name: 'Business Park', location: 'Yakkasaroy, Bobur', commute: 'Metro 12 daqiqa', price: '520 mln so\'m', developer: 'Universal', img: '/tashkent/new-business.jpg' },
  { id: 5, name: 'City Towers', location: 'Yashnabad, Tashkent City', commute: 'Metro 5 daqiqa', price: '680 mln so\'m', developer: 'Tashkent City', img: '/tashkent/city-2025.jpg' },
  { id: 6, name: 'Skyline Plaza', location: 'Sergeli, Yangi Toshkent', commute: 'Metro 15 daqiqa', price: '290 mln so\'m', developer: 'Gross', img: '/tashkent/aerial.jpg' },
]
const buyListings = [
  { id: 1, title: 'Nest One', price: '1 250 mln so\'m', area: '120 m²', rooms: '4-xonali', metro: 'Amir Temur', commute: '5 daq', photos: 12, img: '/tashkent/finance-center.jpg' },
  { id: 2, title: 'Mall Towers', price: '980 mln so\'m', area: '89 m²', rooms: '3-xonali', metro: 'Bunyodkor', commute: '3 daq', photos: 8, img: '/tashkent/mall-towers.jpg' },
  { id: 3, title: 'Finance Center', price: '1 450 mln so\'m', area: '139 m²', rooms: '4-xonali', metro: 'Tashkent City', commute: '5 daq', photos: 15, img: '/tashkent/nest-one.jpg' },
  { id: 4, title: 'Business Park', price: '670 mln so\'m', area: '65 m²', rooms: '2-xonali', metro: 'Bobur', commute: '12 daq', photos: 6, img: '/tashkent/new-business.jpg' },
  { id: 5, title: 'City Towers', price: '2 100 mln so\'m', area: '180 m²', rooms: '5-xonali', metro: 'Minor', commute: '7 daq', photos: 18, img: '/tashkent/city-night.jpg' },
  { id: 6, title: 'Skyline Plaza', price: '540 mln so\'m', area: '45 m²', rooms: 'Studiya', metro: 'Sergeli', commute: '15 daq', photos: 4, img: '/tashkent/city-2025.jpg' },
]
const rentListings = [
  { id: 1, title: 'Green Business Center', price: '18 mln so\'m', area: '120 m²', rooms: 'Ofis 3-xona', metro: 'Amir Temur', commute: '5 daq', photos: 12, img: '/tashkent/finance-center.jpg' },
  { id: 2, title: 'Mall Towers Ofis', price: '12 mln so\'m', area: '85 m²', rooms: 'Savdo maydoni', metro: 'Bunyodkor', commute: '3 daq', photos: 8, img: '/tashkent/mall-towers.jpg' },
  { id: 3, title: 'Nest One Ofis', price: '25 mln so\'m', area: '139 m²', rooms: 'Ofis 4-xona', metro: 'Tashkent City', commute: '5 daq', photos: 15, img: '/tashkent/nest-one.jpg' },
  { id: 4, title: 'City Towers Studio', price: '7 mln so\'m', area: '45 m²', rooms: 'Studiya', metro: 'Minor', commute: '7 daq', photos: 6, img: '/tashkent/city-2025.jpg' },
  { id: 5, title: 'Warehouse Sergeli', price: '24 mln so\'m', area: '450 m²', rooms: 'Ombor', metro: 'Sergeli', commute: '15 daq', photos: 4, img: '/tashkent/new-business.jpg' },
  { id: 6, title: 'Business Park Loft', price: '9 mln so\'m', area: '60 m²', rooms: 'Studiya', metro: 'Oybek', commute: '8 daq', photos: 9, img: '/tashkent/city-night.jpg' },
]
const articles = [
  { title: 'Tashkent City: 2026-yilda narxlar qanday o\'zgardi', tag: 'Tahlil', date: '10 avg 2026', img: '/tashkent/city-2025.jpg' },
  { title: 'Ofis ijaraga olish: qanday shartnomalar foydali', tag: 'Maslahat', date: '8 avg 2026', img: '/tashkent/finance-center.jpg' },
  { title: 'Yangi biznes-markazlar ichidan eng yaxshilari', tag: 'Reyting', date: '5 avg 2026', img: '/tashkent/nest-one.jpg' },
  { title: 'Kommersiya ko\'chmulki: investitsiya qayerda foydali', tag: 'Investitsiya', date: '2 avg 2026', img: '/tashkent/aerial.jpg' },
]
</script>
