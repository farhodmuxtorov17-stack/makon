  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-neutral-500 mt-1">Avtomatik hisobotlar va eksport</p>
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        <div :class="['w-11 h-11 rounded-2xl flex items-center justify-center mb-3', r.color]">
          <svg
            class="w-5 h-5"
            :class="r.iconColor"
            fill="none"
        ...
        <h3 class="font-semibold text-neutral-900">{{ r.title }}</h3>
        <p class="text-sm text-neutral-500 mt-1">{{ r.description }}</p>
        <div class="flex items-center gap-2 mt-4">
          <button class="btn-secondary btn-sm">PDF</button><button class="btn-secondary btn-sm">XLSX</button><button class="btn-secondary btn-sm">CSV</button>
        </div>
      </div>
    </div>
    <div class="card">
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="e in exports" :key="e.id">
              ...
              <td>
                <span class="badge-neutral">{{ e.format }}</span>
              </td>
              <td><StatusBadge :status="e.status" :dot="true" /></td>
              <td class="text-neutral-500 text-xs">{{ formatDate(e.createdAt) }}</td>
              <td><button v-if="e.fileUrl" class="btn-ghost btn-sm">Yuklab olish</button></td>
            </tr>
          </tbody>
        </table>
      </div>
