<template>
  <div class="min-h-screen bg-white text-slate-900">
    ...
    <nav class="sticky top-0 z-50 bg-white/95 backdrop-blur-lg border-b border-slate-100">
      <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div class="hidden lg:flex items-center gap-8 text-sm font-medium text-slate-500">
          <NuxtLink to="/" class="hover:text-slate-900 transition-colors">Katalog</NuxtLink>
    ...
    <!-- Breadcrumb -->
    <div class="max-w-7xl mx-auto px-6 py-4 flex items-center gap-2 text-sm text-slate-400">
      <NuxtLink to="/" class="hover:text-slate-700 transition-colors">Katalog</NuxtLink>
      <span>›</span>
      <span class="text-slate-400">Ijaraga</span>
      <span>›</span>
      <span class="text-slate-400">Ofis</span>
      <span>›</span>
      <span class="text-slate-700 font-medium">Green Business Center</span>
    </div>
      ...
      <div class="lg:col-span-2 space-y-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div class="bg-slate-50 rounded-2xl p-4 text-center">
            <div class="text-2xl font-bold">120<span class="text-sm font-normal text-slate-400 ml-1">m²</span></div>
            <div class="text-xs text-slate-400 mt-1">Maydon</div>
          </div>
          <div class="bg-slate-50 rounded-2xl p-4 text-center">
            <div class="text-2xl font-bold">3<span class="text-sm font-normal text-slate-400 ml-1">/ 12</span></div>
            <div class="text-xs text-slate-400 mt-1">Qavat</div>
          </div>
          <div class="bg-slate-50 rounded-2xl p-4 text-center">
            <div class="text-2xl font-bold">3</div>
            <div class="text-xs text-slate-400 mt-1">Xonali</div>
          </div>
          <div class="bg-slate-50 rounded-2xl p-4 text-center">
            <div class="text-2xl font-bold">B</div>
            <div class="text-xs text-slate-400 mt-1">Sinfi</div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="border border-slate-200 rounded-2xl overflow-hidden">
            <div v-for="(row, i) in characteristics" :key="row.label" class="flex items-center justify-between px-5 py-3.5 text-sm" :class="i % 2 === 0 ? 'bg-slate-50' : 'bg-white'">
              <span class="text-slate-400">{{ row.label }}</span>
              <span class="font-medium text-slate-700">{{ row.value }}</span>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="rounded-2xl overflow-hidden border border-slate-200 h-72 relative bg-slate-100">
            ...
            <div class="absolute bottom-4 left-4 bg-white/95 backdrop-blur-sm rounded-xl px-4 py-3 shadow-lg">
              <p class="text-sm font-medium text-slate-700">Amir Temur kochasi 88</p>
              <p class="text-xs text-slate-400">Mirobad tumani, Toshkent</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-1">
        <div class="lg:sticky lg:top-20 space-y-4">
          ...
          <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-lg shadow-slate-100">
            <div class="mb-1">
              <div class="text-3xl font-bold text-slate-900">18 000 000 <span class="text-lg font-normal text-slate-400">so'm/oy</span></div>
              <div class="text-sm text-slate-400 mt-1">≈ 150 000 so'm/m² · Depozit: 1 oy</div>
            </div>
            <div class="flex gap-2 mt-5">
              <button @click="openApplication()" class="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-sm font-semibold py-3.5 rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all shadow-lg shadow-blue-500/25">
                Ijaraga olish
              </button>
              <button @click="showContact = !showContact" class="w-12 h-12 rounded-xl border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-colors text-lg">♡</button>
            </div>
            <button @click="showContact = !showContact" class="w-full mt-2 text-sm font-medium text-slate-600 bg-slate-50 hover:bg-slate-100 py-3 rounded-xl transition-colors">📞 Bog'lanish</button>

            <!-- Contact info -->
            <div v-if="showContact" class="mt-5 pt-5 border-t border-slate-100 space-y-3">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold text-sm">AB</div>
                <div>
                  <p class="text-sm font-medium text-slate-700">Azizbek Karimov</p>
                  <p class="text-xs text-slate-400">Boshqaruvchi</p>
                </div>
              </div>
              <div class="flex items-center gap-2 text-sm text-slate-600">
                <span class="text-slate-400">📱</span> +998 71 123 45 67
              </div>
              <div class="flex items-center gap-2 text-sm text-slate-600">
                <span class="text-slate-400">✉️</span> info@makon.uz
              </div>
            </div>
          </div>
          ...
          <div class="bg-white border border-slate-200 rounded-2xl p-5">
            ...
            <div class="space-y-2.5 text-sm">
              <div class="flex justify-between"><span class="text-slate-400">Bino nomi</span><span class="font-medium">Green Business Center</span></div>
              <div class="flex justify-between"><span class="text-slate-400">Sinfi</span><span class="font-medium">B sinf</span></div>
              <div class="flex justify-between"><span class="text-slate-400">Qavatlar soni</span><span class="font-medium">12</span></div>
              <div class="flex justify-between"><span class="text-slate-400">Yil</span><span class="font-medium">2024</span></div>
              <div class="flex justify-between"><span class="text-slate-400">Parkovka</span><span class="font-medium">150 o'rin</span></div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div v-else>
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <div>
              ...
              <p class="text-xs text-slate-400 mt-0.5">Green Business Center — A blok, 3-qavat</p>
            </div>
...
const propertyAmenities = [
  'Markaziy konditsioner', '24/7 xavfsizlik', 'Parkovka', 'Panorama oynalar',
  'Mebel bilan jihozlangan', 'Alohida sanuzellar', 'Internet', 'Signalizatsiya',
  'Lift', 'Reception', 'Kafega yaqin', 'Metroga yaqin',
]
</script>
