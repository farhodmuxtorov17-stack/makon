    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <div v-for="s in statCards" :key="s.label" class="card p-5">
        <div class="flex items-center justify-between mb-4">
          ...
          <span v-if="s.trend" class="text-xs font-semibold" :class="s.trendColor">{{
            s.trend
          }}</span>
        </div>
    ...
    <div class="flex gap-1 bg-ink-100 p-1 rounded-xl w-fit">
      ...
      >
        ...
        <span
          ...
          :class="activeTab === t.id ? 'bg-brand-100 text-brand-700' : 'bg-ink-200 text-ink-500'"
        >{{ t.count }}</span>
      </button>
    </div>
        ...
        <div class="flex flex-col md:flex-row">
          <div class="md:w-56 h-44 md:h-auto flex-shrink-0 relative overflow-hidden bg-ink-100">
            ...
            <div class="absolute top-3 left-3">
              <span
                class="badge"
                :class="u.status === 'Faol' ? 'badge-success' : 'badge-warning'"
              >{{ u.status }}</span>
            </div>
          </div>
            ...
            <div class="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-ink-100">
              ...
              <div>
                ...
                <div
                  ...
                  :class="u.paymentStatus === 'Toʻlangan' ? 'text-emerald-600' : 'text-rose-500'"
                >
                  <component
                    :is="u.paymentStatus === 'Toʻlangan' ? CheckCircle2 : AlertCircle"
                    :size="14"
                    :stroke-width="2"
                  />
                  {{ u.paymentStatus }}
          ...
          <template v-for="(step, i) in a.timeline" :key="i">
            <div class="flex items-center gap-2">
              ...
              <span
                ...
                :class="step.done ? 'text-ink-700 font-medium' : 'text-ink-400'"
              >{{ step.label }}</span>
            </div>
            <div
              v-if="i < a.timeline.length - 1"
              class="flex-1 h-px"
              :class="step.done ? 'bg-emerald-200' : 'bg-ink-200'"
            ></div>
          </template>
        </div>
      </div>
          ...
          <tbody>
            <tr v-for="(p, i) in myPayments" :key="i">
              ...
              <td class="text-right">
                <span
                  class="badge"
                  :class="p.status === 'Toʻlangan' ? 'badge-success' : 'badge-danger'"
                >
                  <component
                    :is="p.status === 'Toʻlangan' ? CheckCircle2 : AlertCircle"
                    :size="12"
                    :stroke-width="2.5"
                  />
                  {{ p.status }}
...
const myUnits = [
  {
    id: 1,
    name: 'Green Business Center — A blok, 3-qavat',
    address: 'Mirobad, Amir Temur 88',
    type: 'Ofis',
    status: 'Faol',
    price: '18 000 000 soʻm/oy',
    area: '120 m²',
    floor: '3/12 qavat',
    contractDate: '01.06.2026',
    endDate: '31.05.2027',
    paymentStatus: 'Toʻlangan',
    img: '/tashkent/finance-center.jpg',
  },
  {
    id: 2,
    name: 'Nest One — B blok, 7-qavat',
    address: 'Yashnabad, Tashkent City',
    type: 'Ofis',
    status: 'Faol',
    price: '25 000 000 soʻm/oy',
    area: '139 m²',
    floor: '7/18 qavat',
  ...
  {
    id: 3,
    name: 'Mall Towers — Savdo maydoni',
    address: 'Chilonzor, Bunyodkor kochasi',
    type: 'Savdo',
    status: 'Muddati tugayotgan',
    price: '12 000 000 soʻm/oy',
    area: '85 m²',
    floor: '1/8 qavat',
    contractDate: '01.09.2025',
    endDate: '31.08.2026',
    paymentStatus: 'Toʻlangan',
...
const myApplications = [
  {
    id: 1,
    number: 'APP-2026-0421',
    property: 'Business Park — Loft ofis',
    date: '08.08.2026',
    price: '9 000 000 soʻm/oy',
    duration: '1 yil',
    status: 'Koʻrib chiqilmoqda',
    timeline: [
  ...
  {
    id: 2,
    number: 'APP-2026-0388',
    property: 'City Towers — Studio',
    date: '22.07.2026',
    price: '7 000 000 soʻm/oy',
    duration: '6 oy',
    status: 'Tasdiqlangan',
    timeline: [
      { label: 'Yuborildi', done: true },
...
const myPayments = [
  {
    date: '01.08.2026',
    unit: 'Green BC, 3-qavat',
    period: 'Avgust 2026',
    amount: '18 000 000 soʻm',
    status: 'Toʻlangan',
  },
  {
    date: '15.07.2026',
    unit: 'Nest One, 7-qavat',
    period: 'Iyul 2026',
    amount: '25 000 000 soʻm',
    status: 'Toʻlangan',
  },
  {
    date: '01.07.2026',
