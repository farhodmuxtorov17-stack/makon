<template>
  <div class="space-y-6">
    <NuxtLink
      ...
      class="text-sm text-ink-400 hover:text-ink-600 flex items-center gap-1.5 group"
    >
      <ArrowLeft
        :size="16"
        :stroke-width="2"
    ...
    <div v-if="building" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="card overflow-hidden animate-fade-in-up">
          <div class="relative h-56 flex items-center justify-center overflow-hidden">
            ...
            <!-- Glow -->
            <div
              class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 rounded-full opacity-30 blur-3xl"
              :style="{ background: buildingColor }"
            />
            <!-- 3D Building -->
            ...
            <!-- Badge -->
            <div
              class="absolute top-4 left-4 px-3 py-1.5 rounded-xl bg-white/80 backdrop-blur-sm text-xs font-bold text-ink-600 flex items-center gap-1.5"
            >
              <svg
          ...
          <div class="p-6">
            <div class="flex items-start justify-between mb-5">
              <div>
                <h1 class="text-2xl font-bold font-display text-ink-900 tracking-tight">
                  {{ building.name }}
                </h1>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div
                class="text-center p-4 rounded-2xl bg-gradient-to-br from-brand-50 to-brand-50/50"
              >
                <p class="text-xs text-ink-400 font-medium uppercase tracking-wider">Maydon</p>
                <p class="text-xl font-bold text-ink-900 font-display mt-1">
              ...
              <div
                class="text-center p-4 rounded-2xl bg-gradient-to-br from-emerald-50 to-emerald-50/50"
              >
        ...
        <div class="card animate-fade-in-up">
          ...
          <div class="p-5 grid grid-cols-4 md:grid-cols-6 gap-2">
            <div
              v-for="f in floors"
              :key="f.id"
              :class="[
                'p-3.5 rounded-xl border-2 cursor-pointer text-center transition-all',
                activeFloor === f.id
        ...
        <div class="card animate-fade-in-up">
          ...
          <div class="p-5">
            <svg
              viewBox="0 0 400 260"
              class="w-full h-auto rounded-xl bg-ink-50/50 border border-ink-100"
            >
              <!-- Outer walls -->
              <rect
                x="10"
              ...
              <!-- Door arcs -->
              <path
                d="M 190 130 a 10 10 0 0 1 10 -10"
                fill="none"
                stroke="#8690a8"
                stroke-width="1"
              />
              <path
                d="M 190 130 a 10 10 0 0 0 -10 -10"
                fill="none"
                stroke="#8690a8"
                stroke-width="1"
              />
            ...
            <div class="flex flex-wrap gap-3 mt-4">
              <div
                v-for="(room, i) in sketchRooms"
                :key="'l' + i"
                class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-ink-50"
              >
            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="u in units" :key="u.id">
                  ...
                  <td>
                    <span class="badge badge-neutral">{{ usageLabel(u.usageType) }}</span>
                  </td>
                  <td>
                    <span
                      :class="u.offerType === 'RENT' ? 'badge badge-info' : 'badge badge-success'"
                    >{{ offerLabel(u.offerType) }}</span>
                  </td>
                  <td class="font-mono font-semibold text-ink-900">
                    {{ formatNumber(u.basePrice) }}
                  </td>
      ...
      <div class="space-y-6">
        ...
        <div class="flex flex-col gap-2">
          ...
          <button class="btn btn-ghost text-rose-600 hover:bg-rose-50">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
...
const usageLabel = (t: string) =>
  ({ OFFICE: 'Ofis', RETAIL: 'Savdo', WAREHOUSE: 'Ombor', RESIDENTIAL: 'Turar' })[t] || t
const offerLabel = (t: string) => ({ RENT: 'Ijara', SALE: 'Sotuv' })[t] || t
const typeLabel = (t: string) =>
  ({ COMMERCIAL: 'Kommersiya', RESIDENTIAL: 'Turar', MIXED: 'Aralash' })[t] || t
</script>
