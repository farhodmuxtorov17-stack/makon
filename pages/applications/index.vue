<template>
  <div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3">
      <div>
        <h1 class="text-2xl font-bold font-display text-ink-900">Arizalar</h1>
        <p class="text-sm text-ink-500 mt-1">{{ filtered.length }} ariza</p>
      </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
      <div class="card p-4 flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-brand-50 flex items-center justify-center">
          <ClipboardList :size="18" :stroke-width="2" class="text-brand-600" />
        </div>
        <div>
          <p class="text-xl font-bold">{{ countByStatus('PENDING') }}</p>
          <p class="text-xs text-ink-500">Kutilmoqda</p>
        </div>
      </div>
      <div class="card p-4 flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-warning-50 flex items-center justify-center">
          <ClipboardCheck :size="18" :stroke-width="2" class="text-warning-600" />
        </div>
        <div>
          <p class="text-xl font-bold">{{ countByStatus('FINANCE_REVIEW') }}</p>
          <p class="text-xs text-ink-500">Moliya tekshiruvi</p>
        </div>
      </div>
      <div class="card p-4 flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-accent-50 flex items-center justify-center">
          <Tag :size="18" :stroke-width="2" class="text-accent-600" />
        </div>
        <div>
          <p class="text-xl font-bold">{{ countByStatus('OFFER') }}</p>
          <p class="text-xs text-ink-500">Taklif</p>
        </div>
      </div>
      <div class="card p-4 flex items-center gap-3">
        ...
        <div>
          <p class="text-xl font-bold">{{ countByStatus('APPROVED') }}</p>
          <p class="text-xs text-ink-500">Tasdiqlangan</p>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row gap-3">
      <div class="flex-1">
        <SearchInput v-model="search" placeholder="Ariza raqami yoki mijoz bo'yicha..." />
      </div>
        ...
        <table class="table">
          ...
          <tbody>
            ...
            >
              ...
              <td class="font-medium text-ink-900">{{ app.applicantName }}</td>
              <td>
                <span :class="app.type === 'RENT' ? 'badge badge-info' : 'badge badge-success'">{{
                  app.type === 'RENT' ? 'Ijara' : 'Sotuv'
                }}</span>
              </td>
              <td class="text-ink-500 text-xs">{{ listingTitle(app.listingId) }}</td>
    ...
    <div class="md:hidden space-y-3">
      ...
      >
        ...
        <p class="font-medium text-ink-900">{{ app.applicantName }}</p>
        <div class="flex items-center justify-between mt-2">
          <span :class="app.type === 'RENT' ? 'badge badge-info' : 'badge badge-success'">{{
            app.type === 'RENT' ? 'Ijara' : 'Sotuv'
          }}</span>
          <span class="text-xs text-ink-400">{{ formatDate(app.createdAt) }}</span>
        </div>
      </div>
      <EmptyState
        v-if="!filtered.length"
...
import { ClipboardList, ClipboardCheck, Tag, CheckCircle2, Plus } from 'lucide-vue-next'
...
const filtered = computed(() => {
  ...
  if (search.value)
    r = r.filter(
      (a) =>
        a.number?.toLowerCase().includes(search.value.toLowerCase()) ||
        a.applicantName?.toLowerCase().includes(search.value.toLowerCase())
    )
  return r
})
...
function listingTitle(id: string) {
  return buildingStore.listings.find((l) => l.id === id)?.titleUz || '—'
}
</script>
