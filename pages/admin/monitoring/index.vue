  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-ink-500 mt-1">Tizim holati va xizmatlar</p>
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="s in services" :key="s.name" class="card p-5">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div class="p-3 rounded-xl bg-ink-50">
            <p class="text-xs text-ink-500">Uptime</p>
    ...
    <div class="card">
      <div class="px-5 py-4 border-b border-ink-100">
        <h3 class="font-semibold">Tizim metrikalari</h3>
      </div>
      <div class="p-5 grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div v-for="m in metrics" :key="m.label" class="text-center p-4 rounded-xl bg-ink-50">
          <p class="text-3xl font-bold font-display" :class="m.color">{{ m.value }}</p>
          <p class="text-xs text-ink-500 mt-1">{{ m.label }}</p>
        </div>
      </div>
    </div>
  </div>
...
const metrics = ref([
  ...
  { label: 'Requests/sec', value: '1.2K', color: 'text-brand-600' },
])
function statusClass(s: string) {
  return (
    { healthy: 'badge badge-success', degraded: 'badge badge-warning', down: 'badge badge-danger' }[
      s
    ] || 'badge badge-neutral'
  )
}
